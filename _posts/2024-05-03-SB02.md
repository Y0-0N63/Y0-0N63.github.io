...
**Entity** : 자바 객체를 데이터베이스가 이해할 수 있도록 하는 도구로 이를 기반으로 데이터베이스의 테이블을 생성한다.
데이터베이스에 쓰일 컬럼이나 다른 엔티티 간의 연간 관계를 정의한다.
데이터베이스의 테이블이 사실상 하나의 엔티티기 때문에 실제 데이터베이스와 1:1 매핑된다.
