...
**주요 코드**
3. ```var random = Math.floor(Math.random() *  45) +  1;```
...
4. ```lottoNumbers.splice(random, 1);```
...
5. ```selectedNumbers.sort(function  (a, b)  {return a - b;});```
...
6. ```var lottoNumbersText = selectedNumbers.filter(Boolean).join(' , ');```
...
**주요 코드**
3. ```arrNum.push(parseInt(inputNumStr[i]));```
