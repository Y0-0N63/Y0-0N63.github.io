...
**정의**
값 객체 <br>
단순히 데이터를 저장하기 위한 객체이다. <br>
VO는 특정 값을 가지고 있으며, 보통 데이터의 캡슐화와 불변성을 보장한다. <br> 
주로 데이터의 묶음을 표현하고, 객체 간의 데이터를 전달할 때 사용된다. <br>

**구성 요소**
1. 속성 (fields)
  - 일반적으로 불변 객체(Immutable Object)로, 생성자를 통해 속성을 초기화하며 이후에는 수정되지 않는다.
  - VO는 보통 읽기 전용 속성으로 구성됩니다.

2. 메서드 (methods)
  - 보통 데이터 접근용 getter 메서드만을 제공한다.
  - VO는 상태를 변경하지 않기 때문에 setter 메서드는 제공되지 않는 경우가 많다.
...
**VO를 사용하는 이유**
- **불변성 보장** : VO는 데이터를 불변으로 만들어 데이터의 무결성을 보장한다.
- **캡슐화** : 관련된 데이터를 하나의 객체로 묶어 관리할 수 있다.
-  **단순성** : 데이터를 전달하거나 비교할 때 단순한 구조로 유용하다.
